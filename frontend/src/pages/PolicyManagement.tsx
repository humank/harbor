  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-heading text-2xl font-bold text-text">Policy Management</h1>
      </div>

      {error && <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}

      <Card>
        <h2 className="font-heading mb-4 text-lg font-semibold text-text">Communication Rules</h2>
        ...
        {loading ? (
          ...
          <div className="mb-6 overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-bg-hover rounded-t-xl text-text-muted">
                  <th className="rounded-tl-xl px-4 py-3 font-medium">Rule ID</th>
                  <th className="px-4 py-3 font-medium">From</th>
                  <th className="px-4 py-3 font-medium">To</th>
                  <th className="px-4 py-3 font-medium">Allowed</th>
                  <th className="rounded-tr-xl px-4 py-3 font-medium">Conditions</th>
                </tr>
              </thead>
              <tbody>
                {rules.map(r => (
                  <tr key={r.rule_id} className="border-b border-border transition-colors duration-200 hover:bg-bg-hover">
                    <td className="px-4 py-3 font-mono text-text">{r.rule_id}</td>
                    <td className="px-4 py-3 text-text">{r.from_agent}</td>
                    <td className="px-4 py-3 text-text">{r.to_agent}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-block rounded-lg px-2.5 py-1 text-xs font-semibold ${r.allowed ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                        {r.allowed ? 'Allowed' : 'Denied'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-text-muted">{r.conditions.length ? r.conditions.join(', ') : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Card>
        <h2 className="font-heading mb-2 text-lg font-semibold text-text">Capability Policies</h2>
        <p className="text-sm text-text-muted">Capability policy editor coming soon.</p>
      </Card>

      <Card>
        <h2 className="font-heading mb-2 text-lg font-semibold text-text">Schedule Policies</h2>
